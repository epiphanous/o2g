...
class OntologyProcessor(conf: Conf) extends LazyLogging {
  ...
  val defaultFields: Map[IRI, DefaultField] =
    Map(
      vf.createIRI(DEFAULT_PREFIX + "__id") -> DefaultField(XMLSchema.STRING,
                                                            "Unique identifier",
                                                            isRequired = true,
                                                            directives = List("id")),
      vf.createIRI(DEFAULT_PREFIX + "__typename") -> DefaultField(XMLSchema.STRING,
                                                                  "Object Type",
                                                                  directives = List("typename")),
  ...
  def emitDirectives(): Unit = {
    emitLine(
      s"""#Directs the executor to treat the marked field as a primary key.
         ...
         |
         |#Directs the executor to set the marked field value to the name of
         |#the interface or object type the field belongs to
         |directive @typename on FIELD_DEFINITION""".stripMargin)
  }
