...
class OntologyProcessor(conf: Conf) extends LazyLogging {
  ...
  def emitConnections(): Unit = {
    ...
    val labels = sortByLocalName(connectionTypes).flatMap(t => unions.getOrElse(fieldTypes(t), List(t))).map(genIRI)
    blankLine()
...
//          val typeN = genIRI(typeF)
  ...
  case class DefaultField(fieldType: IRI, comment: String, isRequired: Boolean = false, isList: Boolean = false)
