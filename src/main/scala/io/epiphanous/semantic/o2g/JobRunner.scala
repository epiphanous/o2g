...
object JobRunner extends LazyLogging {
  ...
  val parser = new OptionParser[Conf](BuildInfo.name) {
    ...
    opt[String]('i', "input")
      .action((input, conf) => conf.copy(inputs = conf.inputs :+ input))
    ...
    opt[String]('o', "output")
      .action((out, conf) => conf.copy(out = out))
      .valueName("<gqls-file>")
      .required()
      .maxOccurs(1)
    ...
    opt[String]('c', "catalog")
      .action((catalogXml, conf) => conf.copy(catalogXml = catalogXml))
      .valueName("<xml-file>")
      .optional()
      .maxOccurs(1)
      .text("A catalog xml file to resolve import uri's (default: catalog-v001.xml)")

    opt[String]('r', "inverse")
      .action((fieldName, conf) => conf.copy(inverseFieldWhiteList = conf.inverseFieldWhiteList :+ fieldName))
      .valueName("<field-name>")
      .optional()
      .unbounded()
      .text("A full IRI of an rdf field name to generate the inverse for. You can provide multiple --inverse options.")

    opt[String]('p', "default-prefix")
      .action((prefix, conf) => conf.copy(defaultNamespacePrefix = prefix))
      .valueName("<prefix>")
      .optional()
      .maxOccurs(1)
      .text("The default namespace prefix.")

    opt[String]('n', "default-iri")
      .action((iri, conf) => conf.copy(defaultNamespaceIri = iri))
      .valueName("<iri>")
      .optional()
      .maxOccurs(1)
      .text("The default namespace IRI.")

    opt[Boolean]('l', "local-names")
      .action((local, conf) => conf.copy(useLocalNames = local))
      .optional()
      .maxOccurs(1)
